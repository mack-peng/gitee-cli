...
export function registerReleaseCommands(program: Command): void {
  ...
  release
    ...
    .action(async (opts: { repo?: string; page?: string; perPage?: string; json?: boolean }) => {
      ...
      try {
        const releases = await api.list(owner, repo, {
        ...
        for (const r of releases) {
          ...
          console.log(`    by ${r.author?.login || 'unknown'} · ${GiteeClient.formatDate(r.created_at)}`);
          if (r.assets?.length) {
  ...
  release
    ...
    .action(async (opts: {
      ...
      body?: string; draft?: boolean; prerelease?: boolean; json?: boolean;
    }) => {
      const token = getToken();
      if (!token) {
